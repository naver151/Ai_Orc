...
export default function ChatPage({ user, onBack, theme = 'dark', onThemeToggle, fontSize = 'medium', fontLabel = 'M', onFontSizeCycle }) {
  // ── refs ──────────────────────────────────────────────────
  ...
  const handleConfirm = async () => {
    setMode('working')
    setPending('')
    const { agents } = await analyzeRequest(pendingRequest)
    setWorkspace({ agents, request: pendingRequest })
  ...
  const placeholder = '무엇이든 말씀해 보세요...'
