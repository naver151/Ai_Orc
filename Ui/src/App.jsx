...
export default function App() {
  ...
  const handleUserFormSubmit = (userData) => {
    // uid는 UserFormPage에서 생성해 payload에 포함됨
    localStorage.setItem('aiorc_user', JSON.stringify(userData))
    setUser(userData)
    setPage('chat')
    saveSession('chat', userData)
  }
    ...
    <div ref={appRef} style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden', background: 'var(--bg)' }}>
      ...
      {page === 'landing' && (
        <LandingPage
          ...
          onAdmin={() => { setPage('admin'); saveSession('admin', null) }}
          theme={theme}
      ...
      {page === 'admin' && (
        <AdminPage onBack={() => { setPage('landing'); saveSession('landing', null) }} />
      )}
